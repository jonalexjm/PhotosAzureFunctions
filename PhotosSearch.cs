...
namespace PhotosAzure
{
    public static class PhotosSearch
    {
            ...
            ILogger logger)
        {
            ...
            var query = client.CreateDocumentQuery<PhotoUploadModel>(collectionUri, new FeedOptions() { EnableCrossPartitionQuery = true }).Where(p => p.Description.Contains(searchTerm)).AsDocumentQuery();
